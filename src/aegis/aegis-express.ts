...
const expressFetchRoute = (req: Request) => {
 ...
 export const expressFetchResponseTimes = async (req: Request, res: Response) => {
    try {
      expressGetStats(defaults.responseKey ? defaults.responseKey : 'response-times')
        .then((response) => {
          let myStats: Stats[];
          ...
          myStats.push(event);
          expressDumpStats(myStats, defaults.responseKey ? defaults.responseKey : 'response-times');
        })
        .catch((err) => {
          throw err;
        });
   ...
   try {
      expressGetStats(defaults.dailyKey ? defaults.dailyKey : 'daily')
        .then((response) => {
          ...
          expressDumpStats(myStats, defaults.dailyKey ? defaults.dailyKey : 'daily');
        })
        .catch((err) => {
          throw err;
        });
    ...
    try {
      expressGetStats(defaults.hourlyKey ? defaults.hourlyKey : 'hourly')
        .then((response) => {
          ...
          expressDumpStats(myStats, defaults.hourlyKey ? defaults.hourlyKey : 'hourly');
        })
        .catch((err) => {
          throw err;
        });
    ...
    try {
      expressGetStats(defaults.totalKey ? defaults.totalKey : 'total')
        .then((response) => {
          ...
          expressDumpStats(myStats, defaults.totalKey?  defaults.totalKey: 'total');
        })
        .catch((err) => {
          throw err;
        });
